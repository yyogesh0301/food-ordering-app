...
const ListMealByMealTypeComponent = () => {
  ...
  const getMealsByMealTypeId = () => {
    MealService.getMealsByMealTypeId(mealTypeId)
      .then((response) => {
  ...
  return (
    <>
      {dataFetchingComplete && meals.length === 0 && (
        <div className="title-empty-meal-type">
          Sorry, there are no offers for this category
        </div>
      )}
      {meals.length !== 0 && (
              ...
              return (
                <div className="card-meals-by-meal-type" key={meal.id}>
                  ...
                  <div className="meal-info-container">
                    ...
                    <div className="meal-price">{meal.price}Rs</div>
                  </div>
                  <button
                    className="btn-add-to-cart"
                    onClick={() => handleShowMealQuantity(meal)}
                  >
                    Add to cart
                    <svg
      ...
      <Modal
        show={show}
        onHide={handleClose}
        dialogClassName="modalCustomInsert"
